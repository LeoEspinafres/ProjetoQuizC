...
/*   Updated: 2025/04/25 18:51:49 by leonor            */
...
int main()
{
	...
	do
	{
		...
		switch (op)
		{
		case 1: //sistema de pontos mal feito. repensar
			system("cls");
			printf("Vai ter várias perguntas de grande dificuldade, mas que, consoante o tipo, valem\entre 1 a 3 pontos. Quer prosseguir? (S/N)");
			scanf(" %c", &sim);
			if (sim == 's' || sim == 'S') {
                contagem++;
		...
		case 2:
			system("cls");
			printf("Vai ter várias perguntas de grande dificuldade, mas que, consoante o tipo, valem\entre 4 a 6 pontos. Quer prosseguir? (S/N)");
			scanf(" %c", &sim);
			if (sim == 's' || sim == 'S') {
                contagem++;
				fazerPerguntas(op, jogador);
...
void instrucoes()
{
	...
	getch(); //usar outra alternativa, isto é do windows. E é lento
	Sleep(1000);
	system("cls");
}
...
void adicionarQuestoes()
{
	...
	if (tipoPergunta == 'N')
	{
		...
		scanf(" %[^\n]", perguntaAdicionar);
		// tamanhoPerguntaAdicionar = strlen(perguntaAdicionar);   // POSSO USAR O ? COMO DELIMITADOR
...
int contarLinhas(FILE *perguntas)
{
	...
	while (fgets(linha, sizeof(linha), perguntas) != NULL)
		numLinhas++;
	rewind(perguntas); // Voltar ao início do ficheiro porque nao quero fechar para voltar a abrir 
	if (numLinhas == 0)
	{
		printf("Nao existem perguntas no ficheiro\n");
		return -1;
	}
	return numLinhas;
}
...
void fazerPerguntas(int dificuldade, Jogador jogador)
{
	...
	int maxPerguntas = contarLinhas(perguntas); //impedir que o utilizador coloque que quer responder a mais perguntas do que existem no ficheiro
	Perguntas p[100];
	char c;
	printf("As perguntas vao aparecer de forma aleatoria ate ao numero que selecionou\n"); //falta colocar limite de perguntas consoante o documento
	int numPerguntas = 0;
	do
	{
		printf("Quantas perguntas quer responder?\nResposta: ");
		if (scanf("%d", &numPerguntas) == 1 && numPerguntas > 0 && numPerguntas > maxPerguntas)
			break;
		while ((c = getchar()) != '\n');
		printf("\Insira por favor um numero que seja entre 0 até %d:)\n", maxPerguntas);
	} while (1);

	char baralhado[100][tam_perguntas];
	randomPerguntas(perguntas, baralhado); //tive de baralhar antes de as separar lol 
	...
	for (int i = 0; i < numPerguntas; i++)
	{
		...
		{
			sscanf(linha + 1, " %[^,],%[^,],%[^,],%[^,],%[^\n]",p[i].pergunta,p[i].opcoes[0], p[i].opcoes[1], p[i].opcoes[2], p[i].opcoes[3]);
			strcpy(p[i].multiplaCorreta, p[i].opcoes[0]); // 1ª é correta
		}
	}
	for (int j = 0; j < numPerguntas; j++)
	{
		...
		else if (p[j].tipo == '2') // escolha múltipla
		{
			...
			{
				printf("Acertou!\n");
				jogador.pontuacao += dificuldade + 5;
			}
			else
				printf("Falhou.\n");
		}
...
void historico(Jogador jogador)
{
    ...
    if (flag == 0) 
            printf("Nao existe historico para o jogador %s\n", jogador.nome);
    char resposta;
	while (1)
	{
